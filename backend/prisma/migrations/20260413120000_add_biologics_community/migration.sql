...
SELECT
  ...
  (SELECT u."id" FROM "users" u WHERE u."is_admin" = true ORDER BY u."createdAt" ASC LIMIT 1),
  CURRENT_TIMESTAMP(3),
  CURRENT_TIMESTAMP(3),
  false,
  true,
  ...
  true,
  false
WHERE NOT EXISTS (SELECT 1 FROM "communities" c WHERE c."slug" = 'biologics')
  AND EXISTS (SELECT 1 FROM "users" u WHERE u."is_admin" = true LIMIT 1);
